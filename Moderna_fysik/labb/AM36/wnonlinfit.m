...
%
%   [parameters errors chi2dof]=wnonlinfit(x,y,xerr,yerr,toterr,func,beta0,options)
%   
%
% x,y,xerr,yerr and toterr are the data variables.
% 
...
% Options:
% 
% Options are entered like: wnonlinfit(x,y,xerr,yerr,toterr,func,beta0,'optionname',value,'optionname2',value2)
% 
% 
% 
...
% 
% 
% position: [left, bottom, width, height]       Defines position of legend. 
...
% 
% 
% REMOVED	 % printchi: 'off' or 'on'                       Defines whether chi^2/dof
...
% 
% % Actual fit
% [beta betaerr chi prob chiminvec]=wnonlinfit(x,y,toterr,fitfunc,beta0...
...
function [beta,betaerr,chi2dof,prob,chiminvec] = wnonlinfit(x,y,xerr,yerr,toterr,func,beta0,varargin);
N=length(beta0);
...
if length(toterr)==1
    toterr=toterr *ones(size(y));
end
if length(x)~=length(y) || length(y)~=length(toterr)
...
if any(toterr==0)
    error('ERROR: Errors with value 0 are not allowed.')
end

if any(y+toterr==y)
    warning('MATLAB:wnonlinfit:small_error_warning','Errors are smaller than machine accuracy.\nThis might lead to very strange results')
end

if any(isnan(ytest)) || any(isinf(ytest))
    error('MATLAB:wnonlinfit:badguess','Please test your function! Infinite or NAN values encountered with your bad starting guesses.')
end
...
if exist('lsqnonlin','file')
    [betavec(1,:) chiminvec(1,1)]=lsqnonlin(@(betav) (y-func(x,betav))./toterr ,beta0,[],[],options);
else
    [betavec(i,:) chiminvec(i,1)]=fminsearch(@(betav) chi2(x,y,toterr,@(y) func(y,betav)),beta0.*(1+((i-51)/100+1)*randn(size(beta0))),options);
...
if chiminvec(1)>dof*chitol
    for i=2:50
        ...
        if exist('lsqnonlin','file')
            [betavec(i,:) chiminvec(i,1)]=lsqnonlin(@(betav) (y-func(x,betav))./toterr ,beta0.*(1+randn(size(beta0))/5),[],[],options);
        else
            [betavec(i,:) chiminvec(i,1)]=fminsearch(@(betav) chi2(x,y,toterr,@(y) func(y,betav)),beta0.*(1+((i-51)/100+1)*randn(size(beta0))),options);
        end
...
if chimin>dof*chitol+sum(y)*eps && testbin==0
    for i=51:250
        ...
        if exist('lsqnonlin','file')
            [betavec(i,:) chiminvec(i,1)]=lsqnonlin(@(betav) (y-func(x,betav))./toterr ,beta0.*(1+((i-51)/100+1)*randn(size(beta0))),[],[],options);
        else
            [betavec(i,:) chiminvec(i,1)]=fminsearch(@(betav) chi2(x,y,toterr,@(y) func(y,betav)),beta0.*(1+((i-51)/100+1)*randn(size(beta0))),options);
        end
...
if printbool==1
    fprintf('   Chi2/dof=%1.2f/%i  p=%1.2f\n',[chimin dof prob])
end

betaerr=errorfit(x,y,toterr,func,beta);
...
if isempty(axisin)
    ...
    axisy1=min(y)-toterr(Iminy) -(max(y)-min(y))/40;
    axisy2=max(y)+toterr(Imaxy) +(max(y)-min(y))/40;
...
%
    hold on
    hE=ploterr(x,y,xerr,yerr,'b.'); 	% Plot with x and y error bars
    if ~isempty(xerr) && any(xerr)
    	xlim([x(1)-2*xerr(1), x(length(x))+2*xerr(length(x))]);
    end

    %Edited
    yerr_min=yerr(find(y==min(y)));
    yerr_max=yerr(find(y==max(y)));
    ylim([min(y)-2*yerr_min(1), max(y)+2*yerr_max(1)]);
    %end Edited
    
    set(hE(1), 'MarkerFaceColor', 'b', 'MarkerSize',6);
    hold off
    xlabel(stringcell{1},'Interpreter','Latex','fontsize',18);
    ylabel(stringcell{2},'Interpreter','Latex','fontsize',18);
    strings=cell(1,N);
    for i=1:N
...
%        'top','fontsize',14,'BackgroundColor',[0.95 0.95 0.95],'EdgeColor','k','Margin',1)
	leg = legend([hE(1), hFit], legendText, 'FontSize', 20);
	if isempty(position)
		position = [0.65, 0.7, 0.2, 0.15];
	end

	set(leg, 'Position', position);
...
% Returns chi^2 value for chi=chi2(x,y,toterr,function)
...
function [betaerr] = errorfit(x,y,toterr,func,beta,chimin)
if nargin>5

    chimin2=chi2(x,y,toterr,@(y) func(y,beta));
...
if nargin<6
    chimin=chi2(x,y,toterr,@(y) func(y,beta));
end
...
if N==1
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1))-chimin-mchi,beta(1))-beta(1));

elseif N==2
    mchi=2.3;

    funcerr=@(x,beta1,beta2) func(x,[beta1 beta2]);
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2))-chimin-mchi,beta(2))-beta(2));
...
elseif N==3
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3))-chimin-mchi,beta(3))-beta(3));
...
elseif N==4
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3),beta(4)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3),beta(4)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3,beta(4)))-chimin-mchi,beta(3))-beta(3));

    betaerr(4)=abs(fzero(@(beta4) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta4))-chimin-mchi,beta(4))-beta(4));

elseif N==5

    mchi=6.1;
    funcerr=@(x,beta1,beta2,beta3,beta4,beta5) func(x,[beta1 beta2 beta3 beta4 beta5]);
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3),beta(4),beta(5)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3),beta(4),beta(5)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3,beta(4),beta(5)))-chimin-mchi,beta(3))-beta(3));

    betaerr(4)=abs(fzero(@(beta4) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta4,beta(5)))-chimin-mchi,beta(4))-beta(4));

    betaerr(5)=abs(fzero(@(beta5) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta5))-chimin-mchi,beta(5))-beta(5));
...
elseif N==6
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3),beta(4),beta(5),beta(6)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3),beta(4),beta(5),beta(6)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3,beta(4),beta(5),beta(6)))-chimin-mchi,beta(3))-beta(3));

    betaerr(4)=abs(fzero(@(beta4) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta4,beta(5),beta(6)))-chimin-mchi,beta(4))-beta(4));

    betaerr(6)=abs(fzero(@(beta5) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta5,beta(6)))-chimin-mchi,beta(5))-beta(5));
    
    betaerr(6)=abs(fzero(@(beta6) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta6))-chimin-mchi,beta(6))-beta(6));
...
elseif N==7
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3),beta(4),beta(5),beta(6),beta(7)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3),beta(4),beta(5),beta(6),beta(7)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3,beta(4),beta(5),beta(6),beta(7)))-chimin-mchi,beta(3))-beta(3));

    betaerr(4)=abs(fzero(@(beta4) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta4,beta(5),beta(6),beta(7)))-chimin-mchi,beta(4))-beta(4));

    betaerr(5)=abs(fzero(@(beta5) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta5,beta(6),beta(7)))-chimin-mchi,beta(5))-beta(5));
    
    betaerr(6)=abs(fzero(@(beta6) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta6,beta(7)))-chimin-mchi,beta(6))-beta(6));
    
    betaerr(7)=abs(fzero(@(beta7) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta(6),beta7))-chimin-mchi,beta(7))-beta(7));
    ...
    betaerr(1)=abs(fzero(@(beta1) chi2(x,y,toterr,@(y) funcerr(y,beta1,beta(2),beta(3),beta(4),beta(5),beta(6),beta(7),beta(8)))-chimin-mchi,beta(1))-beta(1));

    betaerr(2)=abs(fzero(@(beta2) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta2,beta(3),beta(4),beta(5),beta(6),beta(7),beta(8)))-chimin-mchi,beta(2))-beta(2));

    betaerr(3)=abs(fzero(@(beta3) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta3,beta(4),beta(5),beta(6),beta(7),beta(8)))-chimin-mchi,beta(3))-beta(3));

    betaerr(4)=abs(fzero(@(beta4) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta4,beta(5),beta(6),beta(7),beta(8)))-chimin-mchi,beta(4))-beta(4));

    betaerr(5)=abs(fzero(@(beta5) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta5,beta(6),beta(7),beta(8)))-chimin-mchi,beta(5))-beta(5));
    
    betaerr(6)=abs(fzero(@(beta6) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta6,beta(7),beta(8)))-chimin-mchi,beta(6))-beta(6));
    
    betaerr(7)=abs(fzero(@(beta7) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta(6),beta7,beta(8)))-chimin-mchi,beta(7))-beta(7));

    betaerr(8)=abs(fzero(@(beta8) chi2(x,y,toterr,@(y) funcerr(y,beta(1),beta(2),beta(3),beta(4),beta(5),beta(6),beta(7),beta8))-chimin-mchi,beta(8))-beta(8));
