...
%     fprintf(FID, '%8.2f & %8.2f & %8.2f & %8.2f \\\\ ', h_reduced(k), RCmax(k),theta_max(k),V_RC_max(k));
...
D=CD_1*0.5*rho_inf(length(h))*Vminthrust^2*S; %Drag
...
theta_gliding=atan(CD_1/CL_1);
V_gliding=(2*(m_12*g*cos(theta_gliding)/S) ./ rho_gliding * sqrt(K/CD0)).^(1/2);

descent_range=(alt_fin1-h_descent)/tan(theta_gliding);

t_descent=trapz(h,1./(V_gliding*sin(theta_gliding)));
