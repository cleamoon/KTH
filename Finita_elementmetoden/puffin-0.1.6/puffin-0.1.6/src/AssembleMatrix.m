function A = AssembleMatrix(points, edges, triangles, pde, W, time)

% Assemble a matrix on a given mesh (points, edges, triangles)
...
for triangle = triangles
  ...
  for p = midpoints
	...
	integral = feval(pde, u, v, w, du, dv, dw, dx, 0, x, d, time, 1) / 3.0;
	A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
...
for edge = edges
  ...
  for p = gausspoints
	...
	integral = feval(pde, u, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, 1) / 2.0;
	A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
