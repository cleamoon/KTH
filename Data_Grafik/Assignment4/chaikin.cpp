                            ...
                            std::vector<vec3>& Curve)
{
    ...
    Curve.reserve(256 * ControlPolygon.size());
    std::vector<vec3> tmp = ControlPolygon;
    Curve = ControlPolygon;
    while(Curve.size() < MinNumDesiredPoints) {
	    Curve.clear();
		for(size_t i(0);i<tmp.size();i++) {
		    const vec3& LeftPoint = tmp[i];
		    const vec3& RightPoint = tmp[(i+1) % tmp.size()];
	...
	//        }
			const vec3 diff = LeftPoint - RightPoint;
			const float eps = 1/1e8;
			if (diff.x*diff.x + diff.y*diff.y + diff.z*diff.z > eps) {
				Curve.push_back(float(3./4.)*LeftPoint + float(1./4.)*RightPoint);
				Curve.push_back(float(1./4.)*LeftPoint + float(3./4.)*RightPoint);
			} else {
